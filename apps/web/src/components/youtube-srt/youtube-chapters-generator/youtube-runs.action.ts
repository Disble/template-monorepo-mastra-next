...
const YOUTUBE_WORKFLOW_KEY = "chapters-videos-workflow";

export async function getYoutubeRunHistory(params?: Partial<PaginationParams>) {
  ...
  const queryParams = {
    ...
    workflowKey: YOUTUBE_WORKFLOW_KEY,
    limit: pageSize,
    offset,
...
export async function getYoutubeRunByRunId(runId: string) {
  const session = await auth.api.getSession({
    headers: await headers(),
  });
...
export async function deleteYoutubeRunByRunId(runId: string) {
  const session = await auth.api.getSession({
    headers: await headers(),
  });
